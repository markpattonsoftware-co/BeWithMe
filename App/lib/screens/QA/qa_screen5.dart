...
import 'qa_screen6.dart';
/// Parameter for the QA5 option bottom sheet.
enum GuardianQA5SheetType { dailyActivity, health }

/// QA step 5: "Let me know what you want me to take care of your [elder] about." with 2 checkboxes.
class QAScreen5 extends StatefulWidget {
  const QAScreen5({super.key, required this.userRoleIndex});
  ...
  @override
  State<QAScreen5> createState() => _QAScreen5State();
}

class _QAScreen5State extends State<QAScreen5> {
  static const _keyGuardianQA2Elder = 'GuardianQA2Elder';
  static const _keyGuardianQA5DailyActivities = 'GuardianQA5DailyActivities';
  static const _keyGuardianQA5Health = 'GuardianQA5Health';
  ...
  void _showOptionSheet(GuardianQA5SheetType type) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => _QA5OptionSheet(type: type),
    );
  }

  Future<void> _onNextPressed() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_keyGuardianQA5DailyActivities, _dailyActivities);
    await prefs.setBool(_keyGuardianQA5Health, _health);

    if (!mounted) return;
    
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => QAScreen6(userRoleIndex: widget.userRoleIndex),
      ),
    );
                          ...
                          children: [
                            ...
                            _buildCheckbox(
                              ...
                              onChanged: (v) {
                                ...
                                if (checked)
                                  _showOptionSheet(
                                    GuardianQA5SheetType.dailyActivity,
                                  );
                              },
                            ...
                            _buildCheckbox(
                              ...
                              onChanged: (v) {
                                ...
                                if (checked)
                                  _showOptionSheet(GuardianQA5SheetType.health);
                              },
...
class _QA5SheetItem {
  _QA5SheetItem({this.checked = false, this.label = '', required this.time, this.controller});
  bool checked;
  String label;
  String time;
...
class _QA5OptionSheet extends StatefulWidget {
  const _QA5OptionSheet({required this.type});

  final GuardianQA5SheetType type;

  @override
  State<_QA5OptionSheet> createState() => _QA5OptionSheetState();
}

class _QA5OptionSheetState extends State<_QA5OptionSheet> {
  static const _keyGuardianQA2Elder = 'GuardianQA2Elder';
  ...
  final List<_QA5SheetItem> _items = [];
  bool _dailyActivityInitialized = false;

  String _elder = '';
  bool _medicineSectionChecked = false;
  bool _vitaminSectionChecked = false;
  final List<_QA5SheetItem> _medicineItems = [];
  final List<_QA5SheetItem> _vitaminItems = [];
  final List<_HealthMeasurementItem> _healthMeasurements = [];
  final List<_QA5SheetItem> _customMeasurementItems = [];
  ...
  void didChangeDependencies() {
    ...
    if (widget.type == GuardianQA5SheetType.dailyActivity &&
    ...
    if (widget.type == GuardianQA5SheetType.health && !_healthInitialized) {
  ...
  Future<void> _loadElderAndMeasurements() async {
    final prefs = await SharedPreferences.getInstance();
    final elder = prefs.getString(_keyGuardianQA2Elder) ?? 'elder';
    if (!mounted) return;
    final l10n = AppLocalizations.of(context)!;
    setState(() {
      _elder = elder;
      ...
      if (_medicineItems.isEmpty) {
        ...
        _medicineItems.add(_QA5SheetItem(checked: false, label: '', time: _defaultTime, controller: c));
      }
      if (_vitaminItems.isEmpty) {
        final c = TextEditingController();
        _vitaminItems.add(_QA5SheetItem(checked: false, label: '', time: _defaultTime, controller: c));
      }
    });
  }
  ...
  void _removeItem(_QA5SheetItem item, [List<_QA5SheetItem>? fromList]) {
    if (!item.isNew) return;
    item.controller?.dispose();
    setState(() => (fromList ?? _items).remove(item));
  }

  Future<void> _pickTime(_QA5SheetItem item) async {
  ...
  void _addMedicineItem() {
    final controller = TextEditingController();
    setState(() {
      _medicineItems.add(_QA5SheetItem(
        checked: false,
        label: '',
        time: _formatTime(DateTime.now()),
        controller: controller,
  ...
  void _addVitaminItem() {
    final controller = TextEditingController();
    setState(() {
      _vitaminItems.add(_QA5SheetItem(
        checked: false,
        label: '',
        time: _formatTime(DateTime.now()),
        controller: controller,
  ...
  void _addCustomMeasurementItem() {
    final controller = TextEditingController();
    setState(() {
      _customMeasurementItems.add(_QA5SheetItem(
        checked: false,
        label: '',
        time: _formatTime(DateTime.now()),
        controller: controller,
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final title = widget.type == GuardianQA5SheetType.dailyActivity
                          ...
                          children: [
                            ...
                            Flexible(
                              child: SingleChildScrollView(
                                ...
                                child: widget.type == GuardianQA5SheetType.health
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                l10n.guardianQA5HealthMedicineQuestion(_elder),
        ...
        Row(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                l10n.guardianQA5HealthVitaminsQuestion(_elder),
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
              ),
            ),
          ],
        ...
        const SizedBox(height: 24),
        Text(
          l10n.guardianQA5HealthNeedsHeading(_elder),
          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
        ),
        const SizedBox(height: 12),
  ...
  Widget _buildHealthEntryRow(_QA5SheetItem item, List<_QA5SheetItem> fromList) {
    return Padding(
  ...
  Widget _buildListItem(_QA5SheetItem item, {List<_QA5SheetItem>? fromList}) {
    return Padding(
