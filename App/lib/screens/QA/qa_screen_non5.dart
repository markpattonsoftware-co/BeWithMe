...
class QAScreenNon5 extends StatefulWidget {
  const QAScreenNon5({super.key, required this.userRoleIndex});
  ...
  @override
  State<QAScreenNon5> createState() => _QAScreenNon5State();
}

class _QAScreenNon5State extends State<QAScreenNon5> {
  static const _keyNonGuardianQA5HobbiesData = 'NonGuardianQA5HobbiesData';
  ...
  Future<void> _onNextPressed() async {
    final list = <Map<String, dynamic>>[
      ..._predefinedHobbies.map(
        (item) => {'title': item.label, 'checked': item.checked},
      ),
      ..._customHobbies.map(
        (item) => {
    ...
    await prefs.setString(_keyNonGuardianQA5HobbiesData, jsonEncode(list));
    if (!mounted) return;
    // TODO: navigate to QAScreen7 when available

    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => QAScreenNon6(userRoleIndex: widget.userRoleIndex),
      ),
    );
  }
