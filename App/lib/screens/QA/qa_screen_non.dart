...
import 'qa_screen_non2.dart';

/// QA (Question & Answer) screen. Receives role index (see [UserRole]) and current QA step index.
class QAScreenNon extends StatefulWidget {
  const QAScreenNon({super.key, required this.userRoleIndex, this.qaIndex = 1});
  ...
  @override
  State<QAScreenNon> createState() => _QAScreenNonState();
}

class _QAScreenNonState extends State<QAScreenNon> {
  static const _keyNonGuardianQA1 = 'NonGuardianQA1';
  ...
  Future<void> _onNextPressed() async {
    ...
    await prefs.setString(_keyNonGuardianQA1, _nameController.text.trim());
    if (!mounted) return;
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => QAScreenNon2(userRoleIndex: widget.userRoleIndex),
      ),
    );
  }
